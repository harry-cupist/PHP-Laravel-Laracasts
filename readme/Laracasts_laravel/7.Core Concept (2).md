...
App\Http\Middleware\Authenticate.php를 살펴보면 Illuminate\Auth\Middleware\Authenticate as Middleware를 export 하는 것을 알수 있음. Illuminate\Auth\Middleware\Authenticate.php는 인증에 관한 미들웨어로서의 로직이 구현되어있음. 특히 `handle` 메서드를 통해 다음 미들웨어로 넘길 수 있음.
